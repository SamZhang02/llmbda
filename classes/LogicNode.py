...
class LogicNode:
    ...
    def _get_leaves(self) -> List[str]:
        ...
        return reduce(
            lambda a, b: a + b, [child._get_leaves() for child in self.children]
        )
    ...
    def __str__(self) -> str:
        ...
        return (
            "("
            + self.value
            + f" {reduce(lambda a,b: a+ ' ' + b, [child.__str__() for child in self.children])}"
            + ")"
        )
    ...
    def _get_coursecodes(self) -> List[str]:
        ...
        return list(
            reduce(
                lambda a, b: a + b,
                [child._get_coursecodes() for child in self.children],
            )
        )
    ...
    def _compute_truth(self, rules: Dict[str, bool]) -> bool:
        ...
        if self.value == "&":
            return reduce(
                lambda a, b: a and b,
                [child._compute_truth(rules) for child in self.children],
            )
        else:
            return reduce(
                lambda a, b: a or b,
