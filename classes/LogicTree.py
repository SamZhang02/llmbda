...
class NotWellFormedTree(Exception):
    pass
...
class LogicTree:
    ...
    def from_list(cls, lst: List) -> LogicTree:
        ...
        if not cls.is_well_formed(lst):
            raise NotWellFormedTree(
                f"Cannot instantiate LogicTreee, {lst} is not a well formed tree"
            )
...
if __name__ == "__main__":
    ...
    tree3 = LogicTree.from_list(
        ["|", ["&", "COMP 202", "COMP 250"], ["&", "COMP 202", "COMP 206"]]
    )
    tree4 = LogicTree.from_list(
        ["|", ["&", "COMP 302", "COMP 250"], ["&", "COMP 202", "COMP 206"]]
    )
    ...
    assert (
        LogicTree.is_well_formed(["&", ["|", "COMP 202", "COMP 302"], "COMP 250"])
        is True
    )
    ...
    assert sorted(tree3.get_leaves()) == sorted(
        ["COMP 202", "COMP 250", "COMP 202", "COMP 206"]
    )
